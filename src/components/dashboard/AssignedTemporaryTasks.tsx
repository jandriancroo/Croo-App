  ...
  if (compact) {
    ...
    const renderPill = (item: typeof badgeItems[0]) => (
      <div
        key={item.id}
        onClick={item.onClick}
        className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg overflow-hidden cursor-pointer active:opacity-80 transition-opacity shadow-md min-w-[calc(50%-4px)] max-w-full flex-grow"
        style={{ backgroundColor: `${item.color}10` }}
      >
        <div className="w-1 h-5 rounded-full shrink-0" style={{ backgroundColor: item.color }} />
