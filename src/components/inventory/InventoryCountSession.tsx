    ...
    buildSnapshotRef.current = () => {
      ...
      const itemCounts = items.map(item => {
        ...
        return {
          ...
          // Snapshot preservation: never overwrite an existing frozen snapshot.
          // _costAtCount / _packQuantityAtCount / _innerPackQuantityAtCount are hydrated
          // from the DB row on load (see items query mapping). Only fall back to live
          // values when no snapshot exists yet (new/never-saved item).
