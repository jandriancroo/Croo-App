...
const dedupeAndSortIngredients = (
  ...
  const getName = (ing: BuilderIngredient): string => {
    ...
    return (v?.name || ing.displayName || "").toLowerCase();
  };
...
const RecipeBuilderDialog = ({ open, onOpenChange, locationId, editRecipeId, editBlueprintId }: RecipeBuilderDialogProps) => {
  ...
  const { data: vendorItems } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("id, name, unit, cost_per_unit, blended_price, pack_size, pack_quantity, pack_quantity_override, count_unit, count_units_per_case, is_recipe, recipe_yield_qty, recipe_yield_unit, brand_item_id")
  ...
  const getItemName = (ing: BuilderIngredient) => {
    ...
    const item = vendorItems?.find(i => i.id === ing.ref_id);
    const name = item?.name || ing.displayName;
    return name || `ingredient-${ing.ref_id.slice(0, 6)}`;
  };
