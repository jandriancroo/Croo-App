...
const RecipeRow = ({ item, tagLabel, locationId, brandId, onEditRecipe, posMapping, posItems, onPosLink, onPosUnlink, onUpdateMappingMeta, isPosLinking }: RecipeRowProps) => {
  ...
  const getIngredientCost = (ing: BlueprintIngredient): number | null => {
    if (ing.ingredient_type === "blueprint" && ing.sub_blueprint_id) {
      ...
      // Pipeline 1 lookup — vendor_item_id is the brand template id, which is the conversionMap key
      ...
      // Fall back to existing location fields when no brand conversion exists
      ...
      // Fallback: parse pack_size for total oz — only when no Pipeline 1 conversion
