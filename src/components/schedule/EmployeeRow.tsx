...
function DayCell({
  ...
  return <div ref={setNodeRef} style={{
    touchAction: 'none'
  }} className={`${isCompactMode ? 'min-h-[44px] px-0.5 py-0.5' : 'min-h-[80px] p-1.5'} border-r last:border-r-0 border-border transition-colors ${isOver ? "bg-accent/50" : "hover:bg-muted/30"} ${isCompactMode ? 'flex items-center' : ''}`}>
      <div className={`${isCompactMode ? 'flex flex-col gap-0.5 w-full' : 'space-y-1'}`}>
