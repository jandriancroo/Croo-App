...
export function DesktopTimeTrackingTable({
  ...
  // Build flat rows for all employees and days
  const rows: {
    card: any;
    day: string;
    dayPunches: any[];
  ...
  filteredCards.forEach((card) => {
    ...
    weekGroups.forEach(([weekKey, weekData]) => {
      ...
      sortedDays.forEach(([day, dayPunches], dayIdx) => {
        ...
        rows.push({
          ...
          isFirstDayOfEmployee,
          weekTotalHours,
          employeeTotalHours: card.totalHours,
        });
        isFirstDayOfEmployee = false;
      });
    });
  });
  ...
  return (
    <div className="border rounded-lg overflow-hidden">
      <Table>
        ...
        <TableBody>
          {rows.map((row, idx) => {
            const { card, day, dayPunches, isFirstDayOfEmployee, weekTotalHours, isFirstDayOfWeek } = row;
                  ...
                  {isFirstDayOfEmployee ? (
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">{card.totalHours.toFixed(1)} hrs total</div>
                      </div>
                    </div>
                  ) : isFirstDayOfWeek ? (
                    <div className="text-xs text-muted-foreground pl-10">
                      Week: {weekTotalHours.toFixed(1)} hrs
                    </div>
                  ) : null}
                </TableCell>
