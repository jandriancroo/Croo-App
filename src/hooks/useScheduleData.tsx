  ...
  const { data: stableData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [userLocationsResult, allProfilesResult, rolesResult, templatesResult] = await Promise.all([
        ...
        supabase.from("profiles").select(`id, full_name, nickname, profile_photo_url, hourly_wage, display_order, appears_on_schedule, weekly_availability`).eq("is_active", true).eq("appears_on_schedule", true),
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const [
        ...
        supabase.from("availability_requests").select("*").eq("location_id", currentLocation.id).eq("request_type", "unpaid").in("status", ["pending", "approved"]).gte("start_date", format(currentWeekStart, "yyyy-MM-dd")).lte("start_date", format(weekEnd, "yyyy-MM-dd")),
      ...
      if (isTeamMemberContext && canSeeFullSchedule && !scheduleVisibilityLoading && profilesWithRoles.length === 0) {
        ...
        if (shiftUserIds.length > 0) {
          const { data: shiftProfiles } = await supabase.from('profiles').select('id, full_name, nickname, profile_photo_url, hourly_wage, display_order, appears_on_schedule, weekly_availability').in('id', shiftUserIds);
          const { data: roles } = await supabase.from("user_roles").select("user_id, role");
          profilesWithRoles = (shiftProfiles || []).map(profile => {
            const userRole = roles?.find(r => r.user_id === profile.id);
            return { ...profile, weekly_availability: profile.weekly_availability as WeeklyAvailability | null, role: userRole?.role || 'team_member', display_order: profile.display_order ?? 0 };
          });
        }
      }
      ...
      queryClient.prefetchQuery({
        ...
        queryFn: async () => {
          ...
          const [shiftsResult, eventsResult, recurringEventsResult, availabilityResult] = await Promise.all([
            ...
            supabase.from("schedule_events").select("*, event_categories(name, color)").eq("schedule_id", schedule.id),
            supabase.from("schedule_events").select("*, event_categories(name, color)").eq("is_recurring", true).is("schedule_id", null).eq("location_id", currentLocation.id),
            supabase.from("availability_requests").select("*").eq("location_id", currentLocation.id).eq("request_type", "unpaid").in("status", ["pending", "approved"]).gte("start_date", format(weekStart, "yyyy-MM-dd")).lte("start_date", format(weekEndDate, "yyyy-MM-dd")),
          ]);
