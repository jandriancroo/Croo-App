...
/**
 * Get a Monday-based day-of-week index (Mon=0..Sun=6) for a YYYY-MM-DD date string
 * interpreted in the provided timezone.
 */
export const getDayOfWeekIndexForDateStringInTimezone = (
  dateStr: string,
  timezone: string = DEFAULT_TIMEZONE
): number => {
  const weekdayShort = new Intl.DateTimeFormat('en-US', {
    timeZone: timezone,
    weekday: 'short',
  }).format(parseDateStringInTimezone(dateStr, timezone));
...
/**
 * Get the previous calendar day (YYYY-MM-DD) in the provided timezone.
 * Uses a midday anchor to avoid edge cases around midnight.
 */
export const getPreviousDateStringInTimezone = (
  dateStr: string,
  timezone: string = DEFAULT_TIMEZONE
): string => {
  const [y, m, d] = dateStr.split('-').map(Number);
  const dateAtNoon = new Date(y, (m ?? 1) - 1, d ?? 1, 12, 0, 0);
