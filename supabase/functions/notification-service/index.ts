...
function getEmailHeader(title: string, logoUrl?: string, orgName?: string): string {
  ...
  return `<tr><td style="background:linear-gradient(135deg,${primaryColor} 0%,#0d5a65 100%);padding:30px 40px;text-align:center;">${logoHtml}<h1 style="color:#fff;font-size:22px;font-weight:600;margin:0;">${title}</h1>${orgName ? `<p style="color:rgba(255,255,255,0.8);font-size:14px;margin:8px 0 0;">${orgName}</p>` : ''}</td></tr>`;
}

function getEmailFooter(): string {
  return `<tr><td style="background-color:#f8f7f5;padding:24px 40px;border-top:1px solid #e8e5df;text-align:center;"><img src="https://croohq.com/assets/croo-logo-eWOfbANR.png" alt="Croo" style="height:24px;margin-bottom:8px;opacity:0.5;"/><p style="color:#aaa;font-size:11px;margin:0;">© ${new Date().getFullYear()} Croo. All rights reserved.</p></td></tr>`;
}

function getCTAButton(href: string, text: string): string {
  return `<table style="width:100%;"><tr><td style="text-align:center;"><a href="${href}" style="display:inline-block;background:linear-gradient(135deg,${accentColor} 0%,#e06b10 100%);color:#fff;text-decoration:none;padding:14px 32px;border-radius:10px;font-weight:600;font-size:15px;">${text}</a></td></tr></table>`;
}
...
async function notifyNewApplication(payload: any): Promise<Response> {
  ...
  // Get admins/managers for this location
...
async function notifyEmployeeJoined(payload: any): Promise<Response> {
  ...
  // Get managers at these locations
