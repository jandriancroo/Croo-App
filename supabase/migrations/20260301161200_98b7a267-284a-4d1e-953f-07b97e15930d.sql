...
-- Fix check_alerts_sql: remove the stale ALARM TASKS section that references
-- non-existent columns (t.alarm_frequency, t.task_type), which crashes the
-- ENTIRE function and prevents overdue checklists + late arrival alerts.
-- Alarm tasks are already handled by the separate trigger_alarm_tasks_sql() cron.
...
BEGIN
  ...
  LOOP
    ...
    -- NOTE: Alarm tasks are handled by the separate trigger_alarm_tasks_sql() function
    -- via its own cron job. No need to duplicate that logic here.
