...
BEGIN
  ...
  LOOP
    ...
    -- Calculate pace from hourly_data: sum actuals for completed hours + projections for future hours
    v_actual_sum := 0;
    v_remaining_proj := 0;
    ...
    IF v_hourly_data IS NOT NULL AND jsonb_typeof(v_hourly_data) = 'array' THEN
      ...
      LOOP
        -- Parse hour (handles both "11:00" and 11 formats)
        BEGIN
        ...
        IF v_hour_num < local_hours THEN
          -- Completed hour: use actual sales
          v_actual_sum := v_actual_sum + COALESCE((v_entry->>'sales')::NUMERIC, 0);
        ELSIF v_hour_num = local_hours THEN
          -- Current hour: use actual if >30min in, else use projection
          IF local_minutes >= 30 THEN
            v_actual_sum := v_actual_sum + COALESCE((v_entry->>'sales')::NUMERIC, 0);
            -- Add remaining fraction of projected for this hour
            v_remaining_proj := v_remaining_proj + 
              (COALESCE((v_entry->>'projected')::NUMERIC, 0) * (60 - local_minutes)::NUMERIC / 60.0);
          ELSE
