...
DECLARE
  ...
  -- Avg over/under pacing variables
...
BEGIN
  ...
  LOOP
    ...
    -- Calculate pace using avg over/under % method
    v_actual_sum := 0;
    v_remaining_proj := 0;
    ...
    IF v_hourly_data IS NOT NULL AND jsonb_typeof(v_hourly_data) = 'array' THEN
      -- First pass: collect per-hour over/under percentages
      ...
      LOOP
        ...
        -- Collect per-hour % for completed hours (and current hour if 30+ min in)
        IF v_hour_projected > 0 THEN
          IF v_hour_num < local_hours AND v_hour_actual > 0 THEN
            v_pct_sum := v_pct_sum + ((v_hour_actual - v_hour_projected) / v_hour_projected);
            v_pct_count := v_pct_count + 1;
          ELSIF v_hour_num = local_hours AND local_minutes >= 30 AND v_hour_actual > 0 THEN
            v_pct_sum := v_pct_sum + ((v_hour_actual - v_hour_projected) / v_hour_projected);
            v_pct_count := v_pct_count + 1;
          END IF;
        END IF;
      ...
      -- Calculate adjustment factor if 3+ hours of data
      IF v_pct_count >= 3 THEN
        v_avg_pct := v_pct_sum / v_pct_count;
        v_severity := LEAST(ABS(v_avg_pct) / 0.50, 1.0);
        IF v_avg_pct < 0 THEN
          v_variant := -(random() * 0.02 * v_severity);
        ELSE
          v_variant := random() * 0.03 * v_severity;
        END IF;
        v_adjustment := 1.0 + v_avg_pct + v_variant;
      END IF;
