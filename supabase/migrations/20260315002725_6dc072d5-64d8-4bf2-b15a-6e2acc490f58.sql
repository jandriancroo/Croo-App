...
DECLARE
  ...
  v_remaining_proj NUMERIC;
  -- Shift-aware pacing variables
  v_hour_actual NUMERIC;
  v_hour_projected NUMERIC;
  v_lunch_pct_sum NUMERIC;
  v_lunch_pct_count INTEGER;
  v_dinner_pct_sum NUMERIC;
  v_dinner_pct_count INTEGER;
  v_active_avg NUMERIC;
  v_is_dinner_shift BOOLEAN;
  v_severity NUMERIC;
  v_variant NUMERIC;
  v_adjustment NUMERIC;
  v_current_hour_contrib NUMERIC;
  v_shift_boundary INTEGER := 15; -- 3 PM
BEGIN
  FOR loc IN
  ...
  LOOP
    ...
    v_actual_sum := 0;
    v_remaining_proj := 0;
    v_current_hour_contrib := 0;
    v_lunch_pct_sum := 0;
    v_lunch_pct_count := 0;
    v_dinner_pct_sum := 0;
    v_dinner_pct_count := 0;
    v_adjustment := 1.0;
    v_is_dinner_shift := (local_hours >= v_shift_boundary);
    
    IF v_hourly_data IS NOT NULL AND jsonb_typeof(v_hourly_data) = 'array' THEN
      -- First pass: collect per-hour over/under % split by shift
      FOR v_entry IN SELECT * FROM jsonb_array_elements(v_hourly_data)
      LOOP
        BEGIN
          IF v_entry->>'hour' LIKE '%:%' THEN
            v_hour_num := split_part(v_entry->>'hour', ':', 1)::INTEGER;
          ELSE
            v_hour_num := (v_entry->>'hour')::INTEGER;
          END IF;
        EXCEPTION WHEN OTHERS THEN
          CONTINUE;
        END;
        
        v_hour_actual := COALESCE((v_entry->>'sales')::NUMERIC, 0);
        v_hour_projected := COALESCE((v_entry->>'projected')::NUMERIC, 0);
        
        IF v_hour_projected > 0 THEN
          -- Check if this hour is completed (or current hour with 30+ min)
          IF (v_hour_num < local_hours AND v_hour_actual > 0) OR
             (v_hour_num = local_hours AND local_minutes >= 30 AND v_hour_actual > 0) THEN
            IF v_hour_num < v_shift_boundary THEN
              v_lunch_pct_sum := v_lunch_pct_sum + ((v_hour_actual - v_hour_projected) / v_hour_projected);
              v_lunch_pct_count := v_lunch_pct_count + 1;
            ELSE
              v_dinner_pct_sum := v_dinner_pct_sum + ((v_hour_actual - v_hour_projected) / v_hour_projected);
              v_dinner_pct_count := v_dinner_pct_count + 1;
            END IF;
          END IF;
        END IF;
      END LOOP;
      
      -- Calculate shift-aware adjustment factor
      v_active_avg := NULL;
      IF v_is_dinner_shift THEN
        IF v_dinner_pct_count >= 3 THEN
          v_active_avg := v_dinner_pct_sum / v_dinner_pct_count;
        ELSIF v_lunch_pct_count >= 3 THEN
          -- Carry lunch avg at 50% weight during dinner ramp-up
          v_active_avg := (v_lunch_pct_sum / v_lunch_pct_count) * 0.5;
        END IF;
      ELSE
        IF v_lunch_pct_count >= 3 THEN
          v_active_avg := v_lunch_pct_sum / v_lunch_pct_count;
        END IF;
      END IF;
      
      IF v_active_avg IS NOT NULL THEN
        v_severity := LEAST(ABS(v_active_avg) / 0.50, 1.0);
        IF v_active_avg < 0 THEN
          v_variant := -(random() * 0.02 * v_severity);
        ELSE
          v_variant := random() * 0.03 * v_severity;
        END IF;
        v_adjustment := 1.0 + v_active_avg + v_variant;
      END IF;
      
      -- Second pass: build pace
      FOR v_entry IN SELECT * FROM jsonb_array_elements(v_hourly_data)
      LOOP
        BEGIN
          IF v_entry->>'hour' LIKE '%:%' THEN
        ...
        IF v_hour_num < local_hours THEN
          v_actual_sum := v_actual_sum + v_hour_actual;
        ELSIF v_hour_num = local_hours THEN
          IF local_minutes >= 30 THEN
            v_actual_sum := v_actual_sum + v_hour_actual;
            v_current_hour_contrib := v_hour_projected * (60 - local_minutes)::NUMERIC / 60.0 * v_adjustment;
            v_remaining_proj := v_remaining_proj + v_current_hour_contrib;
          ELSE
            v_current_hour_contrib := v_hour_projected * v_adjustment;
            v_remaining_proj := v_remaining_proj + v_current_hour_contrib;
          END IF;
        ELSE
          v_remaining_proj := v_remaining_proj + (v_hour_projected * v_adjustment);
        END IF;
      END LOOP;
      
      v_pace := v_actual_sum + v_remaining_proj;
      IF v_pace < v_net_sales THEN
        v_pace := v_net_sales;
      END IF;
    ELSE
      SELECT COALESCE(sc.living_projection, sc.override_projection, sc.initial_projection, sc.projected_sales, 0)
      INTO v_pace
      FROM sales_cache sc
      WHERE sc.location_id = loc.id AND sc.sale_date = local_date::DATE
      LIMIT 1;
