...
-- Fix cost calculation: don't divide by count_units_per_case when ingredient is in Case/Pack units
...
BEGIN
  ...
  SELECT
    ...
    CASE 
      ...
      THEN 
        CASE
          -- If ingredient unit is a Case/Pack (purchasing unit), qty is already in cases
          -- so cost = qty * cost_per_unit (no further division needed)
          WHEN lower(rt_final.uom) LIKE 'case%' OR lower(rt_final.uom) LIKE 'pack%'
          THEN ROUND(rt_final.qty * ii.cost_per_unit, 4)
          -- Otherwise ingredient is in sub-units (oz, grams, each, etc.)
          -- so cost = qty * (cost_per_unit / count_units_per_case)
