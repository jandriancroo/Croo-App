...
-- Fix: only "Case" is case-level pricing. "Pack" is a sub-unit (1 bag out of a case).
...
BEGIN
  ...
  SELECT
    ...
    CASE 
      ...
      THEN 
        CASE
          -- Only "Case" is case-level (qty is in cases, cost_per_unit is per case)
          WHEN lower(rt_final.uom) LIKE 'case%'
          THEN ROUND(rt_final.qty * ii.cost_per_unit, 4)
          -- Everything else (Pack, oz, grams, each, etc.) is a sub-unit
