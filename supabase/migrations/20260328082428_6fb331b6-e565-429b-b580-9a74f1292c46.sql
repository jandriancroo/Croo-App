...
CREATE OR REPLACE FUNCTION public.calculate_theoretical_usage(p_location_id uuid, p_start_date date, p_end_date date)
 RETURNS TABLE(vendor_item_id uuid, vendor_item_name text, ingredient_name text, total_quantity numeric, unit_of_measure text, total_cost numeric, resolution_path text, pos_mapping_name text, units_sold numeric)
 LANGUAGE plpgsql
...
BEGIN
  ...
  FOR v_mapping IN
    ...
    WHERE ipg.location_id = p_location_id
      ...
      AND ipg.blueprint_id IS NOT NULL
  LOOP
    ...
    FROM resolve_recipe_ingredients(v_mapping.blueprint_id, v_units_sold, p_location_id) rri;
  END LOOP;
END;
$function$;
