...
BEGIN
  ...
  LOOP
    ...
    -- Goal: override > living > initial > legacy (matches frontend resolveProjection)
    -- Pace: living_projection tracks real-time pacing; fall back to goal
    SELECT 
      COALESCE(
    ...
    -- Pace should never be less than actual net sales
    ...
    RAISE LOG '[sales-pulse-v4] Queued for % (%): %', loc.name, loc.id, v_body;
  END LOOP;
END;
$$;