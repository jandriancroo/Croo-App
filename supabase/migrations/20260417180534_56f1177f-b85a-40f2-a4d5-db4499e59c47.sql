...
BEGIN
  ...
  -- Authorization: super admin, brand member, or admin/manager (via user_roles) of a location in this brand
  SELECT EXISTS (
    ...
    WHERE ul.user_id = v_caller
      ...
      AND ur.role IN ('admin', 'manager', 'org_admin', 'brand_admin', 'general_manager')
  )
  ...
  IF v_template_id IS NULL THEN
    INSERT INTO brand_inventory_templates (
      ...
      v_blueprint.brand_id, v_blueprint.name, v_category, v_vendor_source, 'active',
  ...
  BEGIN
    ...
    IF v_item_id IS NULL THEN
      INSERT INTO inventory_items (
        location_id, brand_item_id, name, unit, is_recipe, is_active, countable,
