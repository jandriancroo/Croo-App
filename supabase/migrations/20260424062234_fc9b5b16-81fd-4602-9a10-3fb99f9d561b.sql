...
BEGIN
  ...
  -- Existing: Queue backfill_labor for yesterday for every active location
  ...
  -- Existing: labor-service refresh-stale to heal any rows marked stale by auto-punch-out
  PERFORM net.http_post(
  ...
  -- NEW: inventory-availability-sweep across all active locations (empty body = all)
  PERFORM net.http_post(
  ...
  RAISE LOG 'queue_nightly_maintenance: inventory-availability-sweep queued';
END;
$function$;