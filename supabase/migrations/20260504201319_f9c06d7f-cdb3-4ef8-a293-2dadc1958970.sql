...
BEGIN
  ...
  -- Resolve brand_id from location chain when missing (local blueprints)
  IF v_brand_id IS NULL AND v_blueprint.location_id IS NOT NULL THEN
    ...
    IF v_brand_id IS NOT NULL THEN
      ...
      SET brand_id = v_brand_id, updated_at = now()
  ...
  SELECT EXISTS (
    ...
    WHERE bm.user_id = v_caller AND bm.brand_id = v_brand_id
  ) OR EXISTS (
    SELECT 1
  ...
  IF v_template_id IS NULL THEN
    INSERT INTO brand_inventory_templates (
      ...
      v_brand_id, v_blueprint.name, v_category, v_vendor_source, 'live',
...
-- Backfill brand_id on existing local blueprints with null brand_id
UPDATE recipe_blueprints rb
SET brand_id = o.brand_id, updated_at = now()
FROM locations l
JOIN organizations o ON o.id = l.organization_id
WHERE rb.location_id = l.id
  AND rb.brand_id IS NULL
