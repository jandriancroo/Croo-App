...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE ul.user_id = v_caller AND l.brand_id = v_brand_id
  ...
  IF v_template_id IS NULL THEN
    -- Insert as 'draft' to avoid firing trg_auto_deploy_brand_template (which
    -- synchronously fans out an http_post per brand location and times out the txn).
    -- This is a local healing flow; broad deployment is opt-in via the Brand UI.
