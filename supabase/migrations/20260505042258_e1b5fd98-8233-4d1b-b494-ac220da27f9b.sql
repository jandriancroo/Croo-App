...
BEGIN
  ...
  LOOP
    ...
    IF v_pace_pct >= 110 THEN
      v_status := '🔥 On fire';
    ELSIF v_pace_pct >= 100 THEN
      v_status := '✅ Ahead of pace';
    ELSIF v_pace_pct >= 95 THEN
      v_status := '➡️ On pace';
    ELSE
      v_status := '⚠️ Behind pace';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM alerts WHERE dedup_key = v_dedup_key) THEN
      CONTINUE;
    END IF;
    ...
    WHERE ul.location_id = loc.id
      AND ur.role IN ('owner','super_admin','general_manager','assistant_manager');

    IF v_user_ids IS NULL OR array_length(v_user_ids, 1) IS NULL THEN
      CONTINUE;
    END IF;

    INSERT INTO alerts (location_id, alert_type, severity, title, body, dedup_key, target_user_ids, metadata)
    VALUES (
      loc.id,
      'hourly_pulse',
      'info',
      'Hourly Pulse — ' || loc.name,
      v_body,
      v_dedup_key,
      v_user_ids,
      jsonb_build_object('hour', local_hours, 'date', local_date, 'pace_pct', v_pace_pct, 'net_sales', v_net_sales, 'goal', v_goal)
    );
  END LOOP;
END;
$function$;