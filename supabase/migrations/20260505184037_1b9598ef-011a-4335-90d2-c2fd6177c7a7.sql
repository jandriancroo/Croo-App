...
RETURNS TABLE(out_location_id uuid, users_published int, users_skipped int)
...
BEGIN
  ...
  FOREACH loc IN ARRAY _location_ids LOOP
    ...
    FOR u IN SELECT DISTINCT ul.user_id FROM public.user_locations ul WHERE ul.location_id = loc LOOP
      IF EXISTS (
        SELECT 1 FROM public.user_dashboard_cubes c
        WHERE c.user_id = u.user_id AND c.location_id = loc AND c.cube_type = 'tracker'
          AND COALESCE(c.title, '') = COALESCE(_config->>'title', '')
      ) THEN
      ...
      FROM public.user_dashboard_cubes c
      WHERE c.user_id = u.user_id AND c.location_id = loc;
...
RETURNS TABLE(out_location_id uuid, users_updated int, users_created int)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
  caller uuid := auth.uid();
  ...
  FOREACH loc IN ARRAY _location_ids LOOP
    ...
    FOR u IN SELECT DISTINCT ul.user_id FROM public.user_locations ul WHERE ul.location_id = loc LOOP
      IF EXISTS (
        SELECT 1 FROM public.user_dashboard_cubes c
        WHERE c.user_id = u.user_id AND c.location_id = loc AND c.cube_type = 'tracker'
          AND COALESCE(c.title, '') = COALESCE(_original_title, '')
      ) THEN
        UPDATE public.user_dashboard_cubes c SET
          title = _config->>'title',
          accent_color = COALESCE(_config->>'accent_color', c.accent_color),
          tracker_scope = COALESCE(_config->'tracker_scope', c.tracker_scope),
          tracker_display_mode = COALESCE(_config->>'tracker_display_mode', c.tracker_display_mode),
          tracker_item_refs = COALESCE(_config->'tracker_item_refs', c.tracker_item_refs),
          ...
          tracker_location_refs = COALESCE(_config->'tracker_location_refs', c.tracker_location_refs),
          tracker_rank_metrics = COALESCE(_config->'tracker_rank_metrics', c.tracker_rank_metrics),
          updated_at = now()
        WHERE c.user_id = u.user_id AND c.location_id = loc AND c.cube_type = 'tracker'
          AND COALESCE(c.title, '') = COALESCE(_original_title, '');
      ...
      ELSE
        SELECT COALESCE(MAX(c.display_order), -1) + 1 INTO next_order
        FROM public.user_dashboard_cubes c WHERE c.user_id = u.user_id AND c.location_id = loc;
