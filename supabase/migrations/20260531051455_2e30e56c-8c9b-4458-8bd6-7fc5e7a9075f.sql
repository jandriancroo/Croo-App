...
BEGIN
  ...
  SELECT
    ...
    -- Fix A: pack snapshots stamp on every save (not gated on freeze)
    NULLIF(leg->>'pack_quantity_at_count','')::NUMERIC,
    NULLIF(leg->>'inner_pack_quantity_at_count','')::NUMERIC,
    -- Cost + common unit remain freeze-gated (snapshot-wins on submit only)
