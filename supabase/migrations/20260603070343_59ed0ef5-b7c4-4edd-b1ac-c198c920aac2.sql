...
CREATE OR REPLACE FUNCTION public.clone_count_to_sandbox(
...
BEGIN
  ...
  -- Find sandbox location (match by flag + name)
  ...
  -- Wipe any previous sandbox count owned by this user (cascade via deletes)
  ...
  -- Also wipe previously-cloned sandbox storage areas, items, pack selections
  ...
  -- Snapshot source labels for downstream UX
  SELECT name INTO _source_name FROM public.locations WHERE id = _source_location_id;
  SELECT COALESCE(period_type, '') || ' ' || count_date::text
    INTO _source_label
  FROM public.inventory_counts WHERE id = _source_count_id;

  -- Build id maps in temp tables so all clones share the same mapping --
  ...
  -- 1) Clone inventory_locations (storage areas)
  WITH src AS (
  ...
  ins AS (
    INSERT INTO public.inventory_locations (id, location_id, name, sort_order, created_at, updated_at)
    SELECT new_id, _sandbox_location_id, name, sort_order, now(), now()
    FROM src
    RETURNING id
  )
  INSERT INTO _loc_map (old_id, new_id) SELECT id, new_id FROM src;

  -- 2) Clone inventory_items (re-parent storage_location_id via _loc_map)
  WITH src AS (
  ...
  INSERT INTO public.location_pack_selections (location_id, brand_item_id, pack_config_id, selected_at)
  SELECT _sandbox_location_id, brand_item_id, pack_config_id, selected_at
  ...
  -- 5) Clone inventory_count_items (re-parent item_id + storage_location_id)
  WITH src AS (
    SELECT ici.*,
