...
BEGIN
  ...
  -- through the same code paths as the real location. Without this:
  --   * lens_enabled=false  → no brand_pack_configs → inner factor falls
  --     back to inventory_items.inner_pack_quantity (often NULL) → quantity
  --     collapses on the first autosave (e.g. Lemonade 512 → 4).
  --   * legs_enabled=false  → useLegsValuation skips multi-config legs →
  --     Sliced Mushrooms / Mexican Coke rows disappear and item-count drops.
