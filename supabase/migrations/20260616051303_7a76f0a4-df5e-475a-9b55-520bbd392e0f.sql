...
BEGIN
  WITH member_chats AS (
    SELECT cm.chat_id, cm.last_read_at, c.title, c.is_announcement, c.location_id
    FROM chat_members cm
    JOIN chats c ON c.id = cm.chat_id
    WHERE cm.user_id = _user_id AND c.location_id = _location_id
  ),
  latest_messages AS (
    SELECT DISTINCT ON (m.chat_id) m.chat_id, m.sender_id, m.created_at
    FROM messages m
    WHERE m.chat_id IN (SELECT chat_id FROM member_chats)
    ORDER BY m.chat_id, m.created_at DESC
  ),
  unread_chats AS (
    SELECT mc.chat_id, mc.title, mc.is_announcement
  ...
  latest_hiring_msgs AS (
    SELECT DISTINCT ON (hm.conversation_id) hm.conversation_id, hm.sender_type, hm.created_at
  ...
  is_superuser := public.has_role(_user_id, 'super_admin'::app_role);
