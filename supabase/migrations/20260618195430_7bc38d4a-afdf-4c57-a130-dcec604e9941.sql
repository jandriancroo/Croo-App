...
COMMENT ON COLUMN public.pa_catalog_items.pa_product_id IS
  'PA "Product ID" — the value PA prints on pricing sheets, order confirmations, and invoices. Backfilled from pa_orders.items. AUTHORITATIVE resolution key for brand_vendor_mappings.';
...
COMMENT ON COLUMN public.pa_catalog_items.pa_item_id IS
  'LEGACY — currently holds masterProductCode. Kept for backward compatibility; do not use as resolution key. Use pa_product_id instead.';