  ...
  LOOP
    ...
    BEGIN
      ...
      LOOP
        ...
        -- Build the per-item view for this checklist, including a synthetic
        -- section_anchor that ties each non-header row to the nearest preceding header.
        WITH base AS (
          SELECT
            ci.id,
            ci.item_type,
        ...
        scoped AS (
          -- For dynamic checklists, only count non-header items scheduled for today.
          -- Headers are always counted regardless of day.
          SELECT * FROM base
          WHERE
