...
DECLARE
  ...
  tz TEXT;
  local_date TEXT;
  local_day INTEGER;          -- Postgres DOW (Sun=0..Sat=6) — used by dynamic checklist day matching elsewhere
  ...
  start_of_day_utc TIMESTAMPTZ;     -- calendar-day start (kept for late_arrivals/cert)
  end_of_day_utc TIMESTAMPTZ;
  v_dedup_key TEXT;

  -- Business-day window (used by overdue checklist scan only)
  business_local_day_mon0 INTEGER;  -- Mon=0..Sun=6 to match location_hours convention
  today_close TIME;
  close_hour INTEGER;
  cutoff_hour INTEGER;
...
BEGIN
  ...
  LOOP
    ...
    -- ── Business-day window (mirrors getBusinessDayRangeInTimezone in src/utils/timezoneUtils.ts) ──
    ...
    -- Default cutoff is 3 if no close_time is set (matches DEFAULT_CUTOFF_HOUR in the frontend)
    cutoff_hour := CASE WHEN close_hour >= 0 THEN (close_hour + 3) % 24 ELSE 3 END;
    -- Overnight regime only when cutoff lands in early morning (1..11)
    ...
    BEGIN
      ...
      LOOP
        ...
        -- Dedup key now anchored to the business date so it doesn't double-fire across midnight
        v_dedup_key := 'overdue_' || cl.id || '_' || business_date_str || '_h' || local_hours::TEXT;
        IF EXISTS (
