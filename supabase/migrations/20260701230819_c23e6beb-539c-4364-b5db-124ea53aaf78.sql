...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO alert_queue (
      user_ids, location_id, notification_type, title, body, data, dedup_key
    ) VALUES (
      v_user_ids,
      rec.location_id,
      'shift_overstay',
      'Shift overstay',
      v_user_name || ' is still clocked in ' || v_minutes_over || ' min past end of shift at ' || rec.location_name,
      jsonb_build_object('punch_id', rec.punch_id, 'user_id', rec.user_id, 'minutes_over', v_minutes_over),
      v_dedup_key
    );
  END LOOP;
END;
