...
BEGIN
  ...
  -- Match on the super-admin flag only. The name was renamed to
  -- '[TEST] Sandbox (Retired Clone)' and an exact-name match broke the clone.
  WHERE requires_super_admin = true
  LIMIT 1;

  IF _sandbox_location_id IS NULL THEN
    RAISE EXCEPTION 'No Sandbox location found. Set requires_super_admin=true on the sandbox location.';
  END IF;

  -- Mirror resolver gates from source so the sandbox renders + recomputes
  -- through the same code paths as the real location.
  UPDATE public.locations
     SET lens_enabled = (SELECT lens_enabled FROM public.locations WHERE id = _source_location_id),
         legs_enabled = (SELECT legs_enabled FROM public.locations WHERE id = _source_location_id)
   WHERE id = _sandbox_location_id;
  ...
  SELECT
    ...
    i.qubeyond_item_id,
    i.display_order,
    -- Sandbox is a practice environment: force every cloned item active so a
    -- super practicing a count always sees the complete item list, even when
    -- the source store has items deactivated under the new deploy model.
    true AS is_active,
    now(),
    now(),
    i.pack_size,
