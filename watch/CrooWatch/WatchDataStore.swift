...
struct WatchShift: Codable, Hashable, Identifiable {
    ...
    enum CodingKeys: String, CodingKey {
        case id, name, role, time, isMe, status, hours
    }

    init(id: String, name: String, role: String, time: String, isMe: Bool, status: String? = nil, hours: Double? = nil) {
        self.id = id
        self.name = name
        self.role = role
        self.time = time
    ...
    init(from decoder: Decoder) throws {
        ...
        name = (try? c.decode(String.self, forKey: .name)) ?? "Open Shift"
        ...
        if let d = try? c.decode(Double.self, forKey: .hours) {
            hours = d
        } else if let s = try? c.decode(String.self, forKey: .hours) {
            let cleaned = s.replacingOccurrences(of: "h", with: "").trimmingCharacters(in: .whitespaces)
            hours = Double(cleaned)
        } else {
            hours = nil
        }
    }
...
struct WatchSnapshot: Codable {
    ...
    enum CodingKeys: String, CodingKey {
        case updatedAt, locationName, cubes, schedule, shifts, sales, locationId, locations
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        cubes = (try? c.decode([WatchCube].self, forKey: .cubes)) ?? []
        if let s = try? c.decode([WatchShift].self, forKey: .schedule) {
            schedule = s
        } else if let s = try? c.decode([WatchShift].self, forKey: .shifts) {
            schedule = s
        } else {
            schedule = []
        }
        sales = (try? c.decode([WatchMetric].self, forKey: .sales)) ?? []
        locationId = try? c.decode(String.self, forKey: .locationId)
        locations = try? c.decode([WatchLocation].self, forKey: .locations)
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try c.encode(cubes, forKey: .cubes)
        try c.encode(schedule, forKey: .schedule)
        try c.encode(sales, forKey: .sales)
    ...
    private func merge(_ decoded: WatchSnapshot) {
        ...
        if decoded.cubes.isEmpty && !snapshot.cubes.isEmpty {
            merged = WatchSnapshot(
                ...
                schedule: decoded.schedule.isEmpty ? snapshot.schedule : decoded.schedule,
